...
struct Runtime {
    ...
    #[cfg(feature = "lotus")]
    action_waiters: HashMap<String, _lotus::ActionWaiters>,
}
...
mod _lotus {
    use super::*;
    use lotus_script::{action::ActionEvent, input::ActionState, message::Message};

    pub(super) type ActionWaiters = Vec<(Waker, Rc<UnsafeCell<Option<ActionState>>>)>;

    pub fn process_message(message: &Message) {
        get_rt().process_message(message);
    }

    impl Runtime {
        fn process_message(&mut self, message: &Message) {
            let _ = message.handle::<ActionEvent>(|event| {
                if let Some(waiters) = self.action_waiters.remove(&event.name) {
                    // Copy the state to each waiter and wake them
                    for (waker, result_cell) in waiters {
                        unsafe {
                            *result_cell.get() = Some(event.state);
                        }
                        waker.wake();
                    }
                }
